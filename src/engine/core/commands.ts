import type { CityFocus, CityId, DevIncome, PlayerId, ProductionItem, TileId, UnitId } from './types';
...
export type Command =
  ...
  | { type: 'dequeueProduction'; cityId: CityId; index: number }
  | { type: 'reorderProduction'; cityId: CityId; fromIndex: number; toIndex: number }
  | { type: 'setProductionRepeat'; cityId: CityId; repeat: boolean }
  | { type: 'setCityFocus'; cityId: CityId; focus: CityFocus | 'balanced' }
  | { type: 'setRally'; tileId: TileId | null }
  | { type: 'buyProduction'; cityId: CityId; item: ProductionItem }
  | { type: 'setResearch'; techId: string }
  | { type: 'queueResearch'; techId: string }
  | { type: 'dequeueResearch'; techId: string }
