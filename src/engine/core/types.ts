...
export interface Unit {
  ...
  fortified: boolean;
  slept: boolean;
  /** March-to-rally order (P3.4). Absent unless marching. Cleared on arrival, manual orders, or unreachable rally. */
  gotoRally?: boolean;
  xp: number;
  promotions: string[];
}
...
/** Worked-tile preference (P3.3). Balanced is the absence of a focus. */
export type CityFocus = 'growth' | 'production' | 'gold' | 'science' | 'culture';
...
export interface City {
  ...
  /** Repeat unit builds (P3.1). Absent when off — absent beats zeroed for hash cleanliness. */
  productionRepeat?: boolean;
  /** Worked-tile focus (P3.3). Absent means balanced. */
  focus?: CityFocus;
  buildings: string[];
...
export interface Player {
  ...
  /** Research queue (P3.2): tech ids to auto-start on completion. Absent when empty. */
  ...
  /** Rally point for newly produced units (P3.4). Absent when unset. */
  rallyTileId?: TileId;
