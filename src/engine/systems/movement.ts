...
export function applyMoveUnit(state: GameState, events: GameEvent[], cmd: Command): boolean {
  ...
  delete unit.gotoRally; // manual orders take the unit off rally duty
  return true;
}

/**
 * P3.4: march rally-flagged units toward their owner's rally point at turn
 * start. Deterministic (id order, no RNG): arrival, missing rally, parked
 * (fortified/slept) or unreachable goals all clear the flag. unitMoved events
 * are dropped — the narrator ignores them, so rallies march silently.
 */
export function marchRallyUnits(state: GameState, playerId: PlayerId): void {
  const player = state.players[playerId];
  if (!player) return;
  const rally = player.rallyTileId;
  const units = Object.values(state.units)
    .filter((u) => u.ownerId === playerId && u.gotoRally)
    .sort((a, b) => a.id - b.id);
  for (const unit of units) {
    if (unit.fortified || unit.slept || rally === undefined || unit.tileId === rally) {
      delete unit.gotoRally;
      continue;
    }
    const path = findUnitPath(state, unit, rally);
    if (!path) {
      delete unit.gotoRally;
      continue;
    }
    executeMove(state, [], unit, path);
    if (unit.tileId === rally) delete unit.gotoRally;
  }
}
