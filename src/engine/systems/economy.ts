...
import type { City, CityFocus, GameState, Player, PlayerId, ProductionItem, TileId } from '../core/types';
...
export function tileYields(state: GameState, tileId: TileId): Yields {
  ...
  // Rivers feed farms: +1 gold on riverside tiles (SPEC §11).
...
/** P3.3: city focus doubles one yield's weight (balanced = stock weights). */
export function focusYieldScore(y: Yields, focus: CityFocus | undefined): number {
  switch (focus) {
    case 'growth': return y.food * 2.4 + y.production * 1.1 + y.gold * 0.7 + y.science * 1.3 + y.culture;
    case 'production': return y.food * 1.2 + y.production * 2.2 + y.gold * 0.7 + y.science * 1.3 + y.culture;
    case 'gold': return y.food * 1.2 + y.production * 1.1 + y.gold * 1.4 + y.science * 1.3 + y.culture;
    case 'science': return y.food * 1.2 + y.production * 1.1 + y.gold * 0.7 + y.science * 2.6 + y.culture;
    case 'culture': return y.food * 1.2 + y.production * 1.1 + y.gold * 0.7 + y.science * 1.3 + y.culture * 2;
    default: return yieldScore(y);
  }
}
...
export function assignWorkedTiles(state: GameState, city: City): TileId[] {
  ...
  for (const tid of city.ownedTileIds) {
    ...
    candidates.push({ id: tid, score: focusYieldScore(tileYields(state, tid), city.focus) });
  }
...
/** P3.5: production queue cap (head + waiting). */
  ...
  if (item) {
    ...
    if (cost !== undefined) {
      ...
      if (city.productionStored >= cost) {
        ...
        if (item.kind === 'building') delete city.productionRepeat;
        else repeatProduction(state, city, item);
      }
    }
  }
...
function completeItem(state: GameState, events: GameEvent[], city: City, item: ProductionItem): void {
  ...
  unit.movementLeft = 0;
  unit.attacksLeft = 0;
  // P3.4: fresh units join the march when their owner holds a rally point.
  if (state.players[city.ownerId]?.rallyTileId !== undefined) unit.gotoRally = true;
  events.push({ kind: 'productionComplete', cityId: city.id, item });
}

/** P3.1: re-queue a finished unit while repeat is set (buildings are one-shot). */
export function repeatProduction(state: GameState, city: City, item: ProductionItem): void {
  if (!city.productionRepeat) return;
  if (item.kind !== 'unit' || city.productionQueue.length > 0) return;
  if (!canProduce(state, city.ownerId, city, item)) {
    delete city.productionRepeat;
    return;
  }
...
export function processPlayerEconomy(state: GameState, events: GameEvent[], playerId: PlayerId): void {
  ...
  if (tech && player.scienceStored >= tech.cost) {
    ...
    advanceResearchQueue(player);
  }
}

/** P3.2: pull the next unresearched queue entry into the active slot. */
export function advanceResearchQueue(player: Player): void {
  if (!player.researchQueue) return;
  while (player.researchQueue.length > 0) {
    const next = player.researchQueue.shift()!;
    if (!player.researchedTechIds.includes(next)) {
      player.researchingTechId = next;
      break;
    }
  }
