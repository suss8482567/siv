...
const RAID_RADIUS = 6;
const RAIDER_SPAWN_EVERY = 7; // turns per camp
...
function spawnTick(state: GameState): void {
  ...
  const cap = Math.max(2, Math.floor((state.map.width * state.map.height) / 110));
...
function tryNewCamp(state: GameState): void {
  const maxCamps = Math.max(3, Math.min(12, Math.round((state.map.width * state.map.height) / 200)));
