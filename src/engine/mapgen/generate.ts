...
export function generateGame(options: GameOptions): GameState {
  ...
  for (let pid = 0; pid < playerCount; pid++) {
    ...
    // QoL: the escort starts beside the settler, not stacked on it.
    spawnUnit(units, content, nextUnitId++, escortTile(tiles, width, heightRows, starts[pid]), pid, 'warrior');
  }

  // --- barbarian camps: ~1 per 300 tiles, far from the civs ---
  const barbarianCamps: BarbarianCamp[] = [];
  const campCount = Math.max(1, Math.round(n / 300));
...
/** Escort tile: first walkable neighbor of a start (HEX order, deterministic); the start itself when hemmed in. */
function escortTile(tiles: Tile[], width: number, heightRows: number, startId: number): number {
  const s = tiles[startId];
  for (const nb of neighborsOf(s.q, s.r)) {
    const id = tileIndex(nb.q, nb.r, width, heightRows);
    if (id < 0) continue;
    const t = tiles[id];
    if (t.terrain === 'ocean' || t.terrain === 'coast') continue;
    if (t.elevation === 'mountain') continue;
